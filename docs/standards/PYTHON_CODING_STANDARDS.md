...
- **PHI/PII**: Patient names, MRNs, dates of birth, social security numbers.
- **Secrets**: API keys, passwords, private keys (`.pem`, `.key`).
...
**Functionality**:
- Blocks commits containing HL7 patterns (`MSH|...`).
- Blocks commits containing MDS 3.0 identifiers.
- Blocks files with sensitive extensions (`.hl7`, `.pem`, etc.).

**If you are blocked**:
1. Check the error message to locate the offending data.
2. Remove the sensitive data or replace it with synthetic data.
3. If it is a False Positive (e.g., test data), ensure it is properly scrubbed or mocked.
